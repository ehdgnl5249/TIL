...
def solution(genres, plays):
    ...
    for i in total_list:
        total_list[i] = sorted(total_list[i], key=lambda x:(x[0], -x[1]), reverse=True)[:2]
    
    while len(genre_dic) > 0:
        g_max = genre_dic.pop(0)
        if len(total_list[g_max[0]]) > 1:
            answer.append(total_list[g_max[0]][0][1])
            answer.append(total_list[g_max[0]][1][1])
        else:
            answer.append(total_list[g_max[0]][0][1])
    return answer
